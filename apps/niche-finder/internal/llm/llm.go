// Package llm wraps langchaingo's Ollama provider so the rest of the app can ask
// the shared home LLM cluster (via HAProxy) for text and JSON, without caring
// which node actually answers. Same wrapper used across the monorepo's Go apps.
package llm
...
// model is an Ollama model tag (here, a Qwen tag).
...
// Model returns the configured model tag (for logging/UI).
func (c *Client) Model() string { return c.name }
